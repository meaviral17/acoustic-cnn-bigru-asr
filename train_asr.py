...
def run(args):
    ...
    train = LibriCTCDataset(args.data_root, args.train_subset, (encode, decode, stoi, itos, blank), pcen=bool(args.pcen), tf_mixup=bool(args.tf_mixup), specaug=bool(args.specaug), max_len_sec=(args.max_len_sec if args.curriculum else None))
    valid = LibriCTCDataset(args.data_root, args.valid_subset, (encode, decode, stoi, itos, blank), pcen=bool(args.pcen), specaug=False)
    test = LibriCTCDataset(args.data_root, args.test_subset, (encode, decode, stoi, itos, blank), pcen=bool(args.pcen), specaug=False)
    ...
    model = CNNBiGRU(n_mels=train.n_mels, vocab_size=len(vocab), use_se=bool(args.use_se), cnn_channels=args.cnn_channels, num_gru=args.num_gru, gru_hidden=args.gru_hidden, dropout=args.dropout)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        with torch.no_grad():
            ...
            for x,x_lens,ys,y_lens in dl_valid:
                ...
                for seq in pred_ids[:3]:
                    hyps.append(decode(seq))
                ptr = 0
                for L in y_lens.tolist()[:3]:
                    ref = ys[ptr:ptr+L].cpu().tolist()
                    refs.append("".join([train.itos[i] for i in ref]).replace("|"," ").strip())
                    ptr += L
        ...
        if val_loss < best_val:
            best_val = val_loss
            torch.save({"model":model.state_dict(),"stoi":stoi,"itos":itos,"blank":blank}, "experiments/checkpoints/best_cnn_bigru_ctc.pt")
    model.eval()
    if ema: ema.apply_to(model)
    with torch.no_grad():
        hyps, refs = [], []
    ...
    if ema: ema.restore(model)
    if hyps and refs:
        T_WER, T_CER = compute_metrics(refs, hyps)
    else:
